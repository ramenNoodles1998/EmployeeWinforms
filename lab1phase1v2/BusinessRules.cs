...
namespace lab1phase1v2
{
    sealed public class BusinessRules
    {
        ...
        public void UpdateEmployee(double Id, double type, string first, string full, string last, string departmentName, string startDate,string title, string benefits, double wage = 0.0, double rate = 0.0, double sales = 0.0)
        {
            if(type == 0)
            {
                ...
                HourlyEmployee employee = new HourlyEmployee(Id, type, first, full, last,departmentName, title,startDate,benefits,false, wage, rate);
                empData[Id.ToString()] = employee;
            }
            else if(type == 1)
            {
                Salary employee = new Salary(Id, type, first, full, last, departmentName, title, startDate, benefits, false, wage);
                empData[Id.ToString()] = employee;
            }
            else if (type == 2)
            {
                Sales employee = new Sales(Id, type, first, full, last, departmentName, title, startDate, benefits, false, wage, rate, sales);
                empData[Id.ToString()] = employee;
            }
            else
            {
                Contract employee = new Contract(Id, type, first, full, last, departmentName, title, startDate, benefits, false, wage);
                empData[Id.ToString()] = employee;
            }
        }
